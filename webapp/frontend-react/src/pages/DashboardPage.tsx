    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      >
        <div className="sn-page-head-main">
          <p className="sn-eyebrow" style={{ margin: '0 0 6px', color: '#94a3b8' }}>한눈에 보기</p>
        ...
        <div className="sn-page-actions" style={{ gap: 10 }}>
          <button
            onClick={goPassports}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '11px 18px', background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, fontSize: 14, fontWeight: 600, color: '#475569', cursor: 'pointer' }}
          >
          ...
          <button
            onClick={goPassports}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 18px', background: '#1769e0', color: '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: 'pointer', boxShadow: '0 1px 2px rgba(23,105,224,0.1)' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 18, alignItems: 'stretch' }}>
        <div className="sn-panel" style={{ borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(0,0,0,0.04)', background: 'linear-gradient(135deg,#ffffff 0%,#f8fbff 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, marginBottom: 16 }}>
            <p className="sn-eyebrow" style={{ margin: 0, color: '#94a3b8' }}>작업 개요</p>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 999, background: '#eef5ff', color: '#1769e0', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
              지금 먼저 볼 항목
            </span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 12 }}>
            <button onClick={goPassports} style={{ padding: 20, borderRadius: 16, background: '#f8fbff', border: '1px solid rgba(23,105,224,0.08)', textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p className="sn-caption" style={{ margin: 0, fontSize: 14 }}>VIN 연결 필요</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', fontWeight: 800, color: '#0f172a', margin: 0, lineHeight: 1 }}>{bindPend}건</p>
              <p className="sn-caption" style={{ margin: 0 }}>차량 정보 연결이 아직 끝나지 않은 여권</p>
            </button>
            <button onClick={goMaintenance} style={{ padding: 20, borderRadius: 16, background: '#fffaf0', border: '1px solid rgba(245,158,11,0.10)', textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p className="sn-caption" style={{ margin: 0, fontSize: 14 }}>정비·분석 확인</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', fontWeight: 800, color: '#0f172a', margin: 0, lineHeight: 1 }}>{svcOpen}건</p>
              <p className="sn-caption" style={{ margin: 0 }}>결과 등록 또는 후속 검토가 필요한 항목</p>
            </button>
            <button onClick={goRecycling} style={{ padding: 20, borderRadius: 16, background: '#f0fdfa', border: '1px solid rgba(13,148,136,0.10)', textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p className="sn-caption" style={{ margin: 0, fontSize: 14 }}>회수 판단 확인</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '2.5rem', fontWeight: 800, color: '#0f172a', margin: 0, lineHeight: 1 }}>{recycleN}건</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
              {urgentQueue.map((passport) => (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 16px', borderRadius: 12, background: '#fff', border: '1px solid rgba(15,23,42,0.06)', textAlign: 'left', cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}
                >
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 4 }}>{passport.passportId}</div>
        ...
        <div className="sn-panel" style={{ borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(0,0,0,0.04)', background: '#fff' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, marginBottom: 16 }}>
            <div>
              <p className="sn-eyebrow" style={{ margin: 0, color: '#94a3b8' }}>상태 분포</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 14 }}>
            {statusRows.map((row) => (
              <button key={row.key} onClick={() => goStatusRoute(row.key, navigate)} style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '10px 12px', borderRadius: 14, background: '#fbfdff', border: '1px solid rgba(15,23,42,0.04)', cursor: 'pointer', textAlign: 'left' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>{row.label}</span>
                  <span className="sn-caption">{row.value}건</span>
                </div>
                <div style={{ height: 8, borderRadius: 999, background: '#e7eef6', overflow: 'hidden' }}>
