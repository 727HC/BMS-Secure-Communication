app.component('qr-scan-page', {
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-800">카메라 스캔</h2>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          <h2 class="text-sm font-semibold text-gray-800 mb-3">수동 입력</h2>
          <div class="flex gap-2">
            <input v-model="manualId" type="text" placeholder="여권 ID를 입력하세요 (예: BP-SDI-001)"
              @keyup.enter="handleManualSearch"
              class="flex-1 px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none" />
            <button @click="handleManualSearch"
              :disabled="!manualId.trim()"
              :class="['px-4 py-2.5 text-sm font-semibold rounded-lg transition-all',
                !manualId.trim() ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-emerald-600 text-white hover:bg-emerald-700']">
              조회
            </button>
          </div>
        </div>
      ...
      <div>
        <!-- Loading -->
        <div v-if="loadingPassport" class="bg-white rounded-xl border border-gray-200 p-12 flex flex-col items-center justify-center">
          <div class="w-10 h-10 border-[3px] border-emerald-100 border-t-emerald-600 rounded-full animate-spin mb-3"></div>
          <p class="text-sm text-gray-500">여권 정보 조회 중...</p>
        </div>

        <!-- Result Card -->
        <div v-else-if="passportData" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div class="px-5 py-3.5 border-b border-gray-100 bg-emerald-50/50 flex items-center gap-2">
            <svg class="w-4 h-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
            </svg>
            <span class="text-sm font-semibold text-emerald-700">여권 정보 확인됨</span>
          </div>
          <div class="p-5 space-y-4">
            <!-- Model + Status -->
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-bold text-gray-900">{{ passportData.model || '-' }}</h3>
              <span :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border',
                statusColors[passportData.status] || 'bg-gray-50 text-gray-600 border-gray-300']">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                ...
                <p class="text-xs font-mono text-gray-700 mt-0.5 truncate">{{ passportData.passportId }}</p>
              </div>
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                <p class="text-[10px] text-gray-400 uppercase font-medium">시리얼번호</p>
                <p class="text-xs font-mono text-gray-700 mt-0.5">{{ passportData.serialNumber || '-' }}</p>
              </div>
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                <p class="text-[10px] text-gray-400 uppercase font-medium">제조사</p>
                <p class="text-sm font-medium text-gray-700 mt-0.5">{{ passportData.manufacturerName || '-' }}</p>
              </div>
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                <p class="text-[10px] text-gray-400 uppercase font-medium">화학물질</p>
                <p class="text-sm font-medium text-gray-700 mt-0.5">{{ passportData.chemistry || '-' }}</p>
              </div>
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                <p class="text-[10px] text-gray-400 uppercase font-medium">총 에너지</p>
                <p class="text-sm font-medium text-gray-700 mt-0.5">{{ passportData.totalEnergy ? passportData.totalEnergy + ' kWh' : '-' }}</p>
              </div>
              <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                <p class="text-[10px] text-gray-400 uppercase font-medium">VIN</p>
                <p class="text-xs font-mono text-gray-700 mt-0.5 truncate">{{ passportData.vin || '미바인딩' }}</p>
              </div>
            </div>

            <!-- DID -->
            <div v-if="passportData.did" class="bg-emerald-50 rounded-lg px-3 py-2 border border-emerald-100">
              <p class="text-[10px] text-emerald-500 uppercase font-medium">DID</p>
              <p class="text-xs font-mono text-emerald-700 mt-0.5 break-all">{{ passportData.did }}</p>
            </div>
        ...
        <div v-else class="bg-white rounded-xl border border-gray-200 p-12 flex flex-col items-center justify-center text-center">
          ...
          <p class="text-sm font-medium text-gray-500 mb-1">QR 코드를 스캔하거나 여권 ID를 입력하세요</p>
          <p class="text-xs text-gray-400">스캔 결과가 여기에 표시됩니다</p>
        </div>
      </div>
    </div>
  </div>
