...
function createEVManufacturer() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:adminpw@localhost:8054 --caname ca-evmanufacturer --tls.certfiles "${PWD}/organizations/fabric-ca/evmanufacturer/ca-cert.pem"
  { set +x; } 2>/dev/null
...
function createService() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:adminpw@localhost:9054 --caname ca-service --tls.certfiles "${PWD}/organizations/fabric-ca/service/ca-cert.pem"
  { set +x; } 2>/dev/null
...
function createRegulator() {
  ...
  set -x
  fabric-ca-client enroll -u https://admin:adminpw@localhost:10054 --caname ca-regulator --tls.certfiles "${PWD}/organizations/fabric-ca/regulator/ca-cert.pem"
  { set +x; } 2>/dev/null
