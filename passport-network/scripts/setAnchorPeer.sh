...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.manufacturer.battery.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.evmanufacturer.battery.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.service.battery.com"
    PORT=11051
  elif [ $ORG -eq 4 ]; then
    HOST="peer0.regulator.battery.com"
    PORT=13051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.battery.com -c $CHANNEL_NAME -f ${NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
