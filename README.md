...
> NXP S32K3/K1 기반 CAN-FD 보안 BMS와 Hyperledger Fabric 배터리여권(Passport)을 연결하는 통합 플랫폼

임베디드 보안 통신, 블록체인 배터리여권(GBA 21), VC/DID 신원, 웹 콘솔까지 한 저장소에서 관리합니다.
...
| 배터리여권 API 서버 | `bmu-agent/` | Node.js, Express, JWT, Fabric Gateway |
...
# Agent 기동 (시작 시점에 dist/index.html 존재 여부를 확인)
...
## 배터리여권(Passport) API/Web 최신 기준

Passport/API/Web 최신 커밋 기준: `8b5db6e`
검증: `bmu-agent` node:test 40개, `webapp/frontend-react` Vitest 1,270개 통과.

### live chaincode 계약

- Fabric client는 chaincode name `passport-contract`로 호출하며 sequence를 직접 지정하지 않는다.
- live network 기준 `passport-contract` Version `1.4`, Sequence `5`.
- 기존 `CreateBatteryPassport` 인자 순서는 유지한다.
- 3차년도 확장 속성과 BMS binding은 발급 직후 후속 트랜잭션으로 기록한다.
...
rawPayload bytes 44..47: 0c 0e 9a 2c (little-endian)
```

### 실시간 read model / fallback

- 우선 경로는 `cloud-agent` read model이다.
- `cloud-agent:3002`가 꺼져 있어도 `bmu-agent`가 Fabric `QueryPassport` + `QueryBMURecordsByPassport`와 runtime BMU snapshot으로 최신값을 overlay한다.
- Dashboard 개요, Passport detail 개요, BMU 진단 탭은 같은 최신 BMU 기준을 본다.
...
| Tool | 역할 |
|---|---|
| `monitor_transactions` | Fabric/Agent tx 로그, Sequence 5 tx, TPS/성공률 |
...
- Sequence 5 BMS binding tx: `SetPassportExtendedAttributes`, `BindBMSIdentifier`, `RecordSourceVerification`, `RecordBMUDataWithPayload`, 기존 `RecordBMUData`
- Sequence 5 필드: `bmsManagementId`, `bmsBindingId`, `bmsBindingCode32`, `rawPayloadHashVerified`, `physicalVerification.signals.bmsIdentifierMatched`, source verification 최신 상태
...
- [`bmu-agent/README.md`](bmu-agent/README.md) — Passport API, BMU ingest, realtime fallback
