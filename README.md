...
| 영역 | 최신 상태 |
|---|---|
| 배터리여권 API/Web | React 콘솔 + `bmu-agent`가 여권 발급, 3차년도 확장 속성, BMS binding, BMU realtime snapshot을 제공 |
| 블록체인 | `passport-contract` live `version 1.6 / sequence 8`, 4-org Fabric 2.5, 1-of-4 endorsement |
| 임베디드 | MATLAB → `dataProcess.py` → CMU → BMU → bridge → Agent → Fabric E2E 경로 유지 |
| MCP | Fabric/Passport API/감사 로그를 쓰기 없이 관찰하는 read-only 모니터링 |
| KPI | write `205.1 TPS` successful commit, cloud read `2737.9 TPS` |
...
| 영역 | 디렉토리 | 스택 / 책임 |
|---|---|---|
| Hyperledger Fabric 인프라 | `passport-network/` | Fabric 2.5, CouchDB ×4, CA ×5, 4 MSP, benchmark/evaluation profile |
| 체인코드 | `chaincode/passport-contract/` | Go, GBA 21 도메인, MSP RBAC, typed state loader, BMU/VC 검증 |
| Caliper 벤치마크 | `caliper-workspace/` | successful commit write KPI, pre-provisioned passport 검증 |
| 배터리여권 API 서버 | `bmu-agent/` | Node.js, Express, JWT, Fabric Gateway, BMU ingest |
| 오프체인 read model | `cloud-agent/` | MongoDB, block event sync, high-TPS read API |
...
```bash
# Write KPI: successful commit / Succ-only 기준
...
`CALIPER_SKIP_PREPARE=true`는 명시적 `CALIPER_RUN_ID`, `BMU_FC_START`, 사전 준비된 passport/DID 검증을 요구합니다. setup 포함 cold-start TPS는 공식 write KPI가 아니라 별도 진단값입니다.
...
| KPI | 최신 결과 | 판정 기준 |
|---|---:|---|
| Fabric write | `205.1 TPS` | successful commit / Succ-only, `Succ 5000/5000`, `Fail 0`, `Reject 0` |
| Cloud read | `2737.9 TPS` | HTTP → cloud-agent → MongoDB, `Completed 5000`, `Errors 0` |

- Caliper `Throughput`은 참고 지표입니다. 공식 write 수치는 실패 tx를 섞지 않는 **successful commit TPS**입니다.
- 상세 측정 명령, 로그 경로, A/B 결과는 [`wiki/blockchain/benchmark-methodology.md`](wiki/blockchain/benchmark-methodology.md)에 기록합니다.
...
- 현재 live 계약은 `passport-contract` `version 1.6 / sequence 8`입니다.
- BMS binding 계열 tx는 sequence 5에서 도입됐고, 현재 sequence 8에서도 유지됩니다.
...
| Caliper benchmark | `RecordBMUData` successful commit path |
...
- `cloud-agent:3002`가 꺼져 있어도 `bmu-agent`가 Fabric 최신 BMU record와 runtime snapshot으로 화면 값을 보강합니다.
...
| Tool | 역할 |
|---|---|
| `monitor_transactions` | Fabric/Agent tx 로그, TPS/성공률 |
...
- [`wiki/blockchain/benchmark-methodology.md`](wiki/blockchain/benchmark-methodology.md) — KPI 측정 방법론
