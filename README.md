...
### NXP S32K BMU/CMU 보안 통신 ↔ Hyperledger Fabric 배터리여권 통합 플랫폼

차량용(xEV) 배터리관리시스템의 센서 데이터를 보드 레벨 암호화부터 분산원장 기반 배터리여권까지 무결성을 잃지 않고 잇는 엔드투엔드 레퍼런스 구현입니다.
...
<!-- 코어 스택 -->
![Hyperledger Fabric 2.5](https://img.shields.io/badge/Hyperledger_Fabric-2.5-2F3134?logo=hyperledger&logoColor=white)
...
<!-- 임베디드 & 프로토콜 -->
![NXP S32K3 / S32K1](https://img.shields.io/badge/MCU-NXP_S32K344_%2F_S32K144-000000?logo=nxp&logoColor=white)
![CAN-FD](https://img.shields.io/badge/Bus-CAN--FD-FF6F00)
![FreeRTOS](https://img.shields.io/badge/RTOS-FreeRTOS-1FA336)
![Crypto](https://img.shields.io/badge/Crypto-AES--128_CMAC%2FCBC_%C2%B7_Ed25519-6E40C9)

<!-- 표준 & 모델 -->
![GBA](https://img.shields.io/badge/Standard-GBA_Battery_Passport-1F6FEB)
![DID / VC](https://img.shields.io/badge/Identity-DID_%2F_VC-0A66C2)
![MCP](https://img.shields.io/badge/MCP-read--only_monitor-purple)
![Architecture](https://img.shields.io/badge/Pattern-CQRS_read_model-555555)
...
**무엇.** 이 저장소는 차량용 배터리관리시스템(BMS)의 센서 데이터가 발생 시점부터 분산원장 기반 **배터리여권(Battery Passport)** 레코드가 되기까지의 전 경로를 하나의 코드베이스로 묶은 통합 플랫폼입니다. NXP S32K 마이크로컨트롤러(CMU `S32K144` / BMU `S32K344`)에서 CAN-FD 보안 통신으로 측정값을 모으고, BMU가 Ed25519 서명을 붙인 48-byte payload를 `bmu-agent`(Node.js)가 받아 Hyperledger Fabric 4-org 네트워크의 `passport-contract`(Go 체인코드)에 기록합니다. 기록된 원장은 `cloud-agent`가 MongoDB read model로 투영하고, React 웹 콘솔과 read-only `mcp-monitor`가 이를 관찰합니다.

**왜.** 배터리여권은 EU 배터리 규제 등 국제 규범이 요구하는 디지털 제품 이력 체계로, 데이터의 **출처·무결성·검증 가능성**을 동시에 보장해야 합니다. 본 플랫폼은 이를 위해 보드 레벨 암호화(CSEc/HSE 하드웨어 보안)와 분산원장의 변조 저항성을 결합하고, 신원은 DID/VC, 원장은 MSP 기반 권한제어로 다룹니다. 본 저장소는 한국 국가연구개발 과제의 일환으로 개발된 **xEV BMS 보안 플랫폼 레퍼런스 구현**이며, GBA(Global Battery Alliance) 배터리여권 데이터 모델을 도메인 기준으로 참조합니다. 표준이 정의하는 데이터 항목만으로는 *센서에서 원장까지*의 종단간 무결성이 보장되지 않으므로, 본 구현은 하드웨어 신뢰근원(HSE/CSEc) · DID/VC · 분산원장을 **하나의 검증 경로**로 연결해 그 간극을 메우는 데 초점을 둡니다.
...
데이터는 단방향으로 흐릅니다. 물리 센서 입력에서 시작해 보드 레벨 암호화 통신을 거쳐 분산원장에 기록되고, read model을 통해 조회·모니터링 표면으로 투영됩니다. 원장에 **쓰기**가 가능한 경로는 `bmu-agent` 하나뿐이며, `cloud-agent`와 `mcp-monitor`는 **읽기 전용**입니다.
...
README 본문은 네 개의 도메인으로 나뉘며 각 영역은 별도 담당자가 유지합니다.
...
배터리여권 레이어는 React 웹 콘솔과 `bmu-agent` API 서버, 그리고 오프체인 read model(`cloud-agent`)을 묶어 GBA 배터리여권의 발급·바인딩·검증·BMU 실시간 데이터를 사람이 다루는 애플리케이션으로 만듭니다. 체인코드 함수 정의·MSP RBAC·원장 거버넌스는 [블록체인 인프라](#블록체인-hyperledger-fabric-인프라)가 소유하며, 이 섹션은 그 위에 올라가는 **앱 관점**만 다룹니다.

![Passport 운영 콘솔 — 개요(Dashboard)](wiki/passport/assets/page-dashboard.png)
...
> ⚠️ **정정**: 루트에 `build.sh` 는 없다. 빌드는 `./e2e.sh`(또는 각 `Debug_FLASH`에서 직접 `make`), 플래시는 `./flash.sh` 로 한다.
...
**라이선스.** 현재 본 저장소에는 LICENSE 파일이 포함되어 있지 않습니다. 명시적 라이선스가 추가되기 전까지 모든 권리는 저작자에게 유보됩니다. 사용·재배포 조건에 대해서는 메인테이너에게 문의하세요.

**기여.** 본 저장소는 네 개 도메인(블록체인 / 배터리여권 / 임베디드 / MCP)으로 나뉘어 유지됩니다. 변경은 해당 도메인 범위 안에서 작업하고, 도메인 경계를 넘는 변경은 사전 합의 후 진행해 주세요. 데이터 흐름의 정본 경로(`MATLAB → CMU → BMU → bmu-agent → Fabric → cloud-agent → mcp-monitor`)는 임의로 변경하지 않습니다.

<div align="center">
<sub>xEV BMS 보안 통신 · 배터리여권 통합 플랫폼 · 국가연구개발 과제 레퍼런스 구현</sub>
</div>