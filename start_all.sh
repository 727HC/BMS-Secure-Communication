...
# C-3/H-15: ACA-Py seed/wallet key는 fallback 없이 필수 주입 (bmu-agent/.env에서 로드)
: "${ACAPY_SEED:?ACAPY_SEED must be set in bmu-agent/.env}"
: "${ACAPY_WALLET_KEY:?ACAPY_WALLET_KEY must be set in bmu-agent/.env}"
ACAPY_SD="${ACAPY_SEED}"
ACAPY_WN="${ACAPY_WALLET_NAME:-bmu_wallet2}"
ACAPY_WK="${ACAPY_WALLET_KEY}"
