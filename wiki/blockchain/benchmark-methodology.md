...
3. **BENCH 계정 등록** (bmu-agent 경유)
   ...
   curl -s -X POST http://localhost:3001/api/auth/register \
     ...
     -d '{"userId":"bench","password":"benchpass123","role":"manufacturer","orgNum":1}'
   ```
...
6. **벤치 대상 passport 준비** — `scripts/tps-benchmark-cloud.js` 는 `PASSPORT-BMU-DEVICE` 를 하드코딩. 반드시 사전 생성
   ...
   TOKEN=$(curl -s -X POST http://localhost:3001/api/auth/login \
     -H "Content-Type: application/json" \
     -d '{"userId":"bench","password":"benchpass123","orgNum":1}' | jq -r '.token')
...
```bash
cd /home/heechan/bms-blockchain
BENCH_USER=bench BENCH_PASSWORD=benchpass123 BENCH_ORG=1 \
...
| CA bootstrap admin | `passport-network/.env` + `fabric-ca-server-config.yaml` identities | `admin:adminpw` 일치 |
