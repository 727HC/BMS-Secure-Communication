...
int main(void)
{
    ...
    if(FALSE == checkHseFwFeatureFlagEnabled())
    {
    	  ...
    	  /* Fatal: usage flag must be committed before reset */
    	  ASSERT(FLASH_OK == status);
    }
...
boolean checkHseFwFeatureFlagEnabled(void)
{
    ...
    //check against the expected enable pattern, not just "not default"
    uint64_t expected_val = 0xAABBCCDDDDCCBBAAUL;
    if(0 == memcmp((void *)&hsefwfeatureflag, (void *)&expected_val, 0x8U))
    {
        fw_enabled = TRUE;
    }
    return fw_enabled;
