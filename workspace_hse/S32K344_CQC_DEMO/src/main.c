...
#define HSE_NVM_CATALOG \
		{HSE_MU0_MASK,   HSE_KEY_OWNER_CUST, HSE_KEY_TYPE_AES,      10U, 128U            },	\
		{HSE_MU0_MASK,   HSE_KEY_OWNER_CUST, HSE_KEY_TYPE_ECC_PAIR, 10U, HSE_KEY256_BITS },	\
		{0U,             0U,                 0U,                     0U                   }

#define HSE_RAM_CATALOG	\
		{HSE_MU0_MASK,   HSE_KEY_OWNER_ANY,  HSE_KEY_TYPE_AES,      10U, 128U            },	\
...
static volatile uint32_t numberOfResponses = 0;
...
int main(void) {
	...
	/* Format result: OK or NOT_ALLOWED (already formatted) are acceptable.
	 * Any other response is a fatal error. */
	if (gSrvResp == HSE_SRV_RSP_OK)
	{
		/* NVM/RAM key catalogs formatted successfully — continue */
	}
	else if (gSrvResp == HSE_SRV_RSP_NOT_ALLOWED)
	{
		/* Key catalogs already formatted — continue */
	}
	else
	{
	...
	/* Verify ADKP regardless of whether we just programmed it or it was pre-provisioned */
	...
	if(HSE_SRV_RSP_OK == gSrvResp){
		...
		ASSERT(HSE_SRV_RSP_OK == gSrvResp);
		/* Wait for the async hash callback to complete */
		while(numberOfResponses == 0) {}
		ASSERT(HSE_SRV_RSP_OK == asyncResponses[0][1]);
	}
	...
	ASSERT(HSE_SRV_RSP_OK == gSrvResp);
	if((HSE_SRV_RSP_OK == gSrvResp) && (gAdkpKeyTest == ADKP_PROGRAMMING_SUCCESS)){
		if(gHseCurrentLC == HSE_LC_CUST_DEL){
